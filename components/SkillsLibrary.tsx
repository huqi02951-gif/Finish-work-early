...
const HIDDEN_LIBRARY_ENTRY_IDS = new Set([
  'bbs-center-main',
  'bbs-feed',
  'bbs-publish',
]);
...
function getSkillCardCopy(skill: Skill): SkillCardCopy {
  return {
    ...
    action: skill.toolRoute ? '打开工具' : '查看详情',
  };
}
...
const SkillsLibrary: React.FC = () => {
  ...
  const filteredSkills = useMemo(() => {
    const list = skills.filter((skill) => {
      if (HIDDEN_LIBRARY_ENTRY_IDS.has(skill.id)) return false;
