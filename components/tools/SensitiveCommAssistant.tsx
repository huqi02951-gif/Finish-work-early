...
const SensitiveCommAssistant: React.FC = () => {
  ...
  const generateContent = () => {
    ...
    setTimeout(() => {
      ...
      const rawPhone = basicParams.myPhone.trim();
      // 有填号码就显示具体电话，没填就用通用表述
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <div className="relative">
                      <input 
                        type="text" 
                        placeholder="不填则生成通用表述、无具体号码"
                        className="w-full px-4 md:px-6 py-3 md:py-4 pr-20 bg-brand-light-gray/40 border border-brand-border/10 rounded-xl md:rounded-2xl focus:ring-2 focus:ring-brand-gold/20 outline-none transition-all font-medium text-sm md:text-base text-brand-dark"
                        value={basicParams.myPhone}
                        onChange={(e) => handleBasicChange('myPhone', e.target.value)}
                      />
                      {basicParams.myPhone && (
                        <button
                          onClick={() => handleBasicChange('myPhone', '')}
                          className="absolute right-3 top-1/2 -translate-y-1/2 text-[9px] text-brand-gray/40 hover:text-brand-gray font-medium px-2 py-1 rounded-lg hover:bg-brand-light-gray transition-all"
                        >
                          清空
                        </button>
                      )}
                    </div>
                    <p className="mt-1.5 text-[9px] text-brand-gray font-medium">
                      {basicParams.myPhone
                        ? <span className="text-emerald-600 opacity-70">✓ 已填写，话术中将体现为「请致电柜台 {basicParams.myPhone}」</span>
                        : <span className="opacity-30">未填写，生成通用表述「请联系网点柜台」</span>
                      }
                    </p>
                  </div>
                </div>
